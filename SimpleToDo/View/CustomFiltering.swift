...
struct CustomFiltering<Content: View>: View {
    ...
    init(filterData: Binding<Date>, @ViewBuilder content: @escaping ([Task],[Task]) -> Content) {
        ...
        _result = FetchRequest(entity: Task.entity(), sortDescriptors: [
            NSSortDescriptor(keyPath: \Task.date, ascending: false)], predicate: predicate, animation: .easeInOut(duration: 0.25))
...
struct CustomFiltering_Previews: PreviewProvider {
